...
# When we have spinned those stops, we look around again to find the biggest
# cluster of unspinned Pokestops/gyms and move there, and so on!
...
class BallCollector(BaseTask):
    ...
    def work(self):
        ...
        if not self.bot.catch_disabled and balls_on_hand <= self.min_balls:
            # Okay, we need to fillup on balls now, so we notify other workers
            # to disable catching
        ...
        if self.cluster is not None:
            ...
            if self.walker.step():
                ...
                self.emit_event(
                    ...
                    formatted="Arrived at destination: {size} forts.".format(
                        **self.cluster))
                self.cluster = None
                self.previous_distance = 0
                return WorkerResult.SUCCESS
    ...
    def get_available_clusters(self):
        ...
        available_clusters = [
            c for c in self.clusters if c["size"] >= self.config_min_forts_count]
        available_clusters.sort(
            key=lambda c: self.get_cluster_key(c),
            reverse=True)
