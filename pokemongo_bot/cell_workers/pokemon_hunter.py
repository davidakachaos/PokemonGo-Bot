...
class PokemonHunter(BaseTask):
    ...
    def initialize(self):
        ...
        # closest first?
        self.config_hunt_closest_first = self.config.get(
            "hunt_closest_first", False)
        ...
        self.config_target_family_of_vip = self.config.get(
            "target_family_of_vip", True)
        self.config_treat_family_of_vip_as_vip = self.config.get(
            "treat_family_of_vip_as_vip", False)
        self.bot.hunter_locked_target = None
        # Hunt for trash when bags are almost full
        self.config_hunt_for_trash = self.config.get(
            "hunt_for_trash_to_fill_bag", False)
        self.config_trash_hunt_open_slots = self.config.get(
            "trash_hunt_open_slots", 25)
    ...
    def work(self):
        ...
        if self.bot.catch_disabled:
            ...
            if not hasattr(
                self.bot,
                "hunter_disabled_global_warning") or (
                hasattr(
                    self.bot,
                    "hunter_disabled_global_warning") and not self.bot.hunter_disabled_global_warning):
                self.logger.info(
                    "All catching tasks are currently disabled until {}."
                    " Pokemon Hunter will resume when catching tasks are re-enabled".format(
        ...
        if self.bot.softban:
            ...
            if not hasattr(
                self.bot,
                "softban_global_warning") or (
                hasattr(
                    self.bot,
                    "softban_global_warning") and not self.bot.softban_global_warning):
                self.logger.info(
        ...
        if self.no_hunt_until is not None and self.no_hunt_until > time.time():
        ...
        if hasattr(self.bot, "hunter_locked_target"):
            if self.destination is not None and self.bot.hunter_locked_target is not None:
                if self.destination is not self.bot.hunter_locked_target:
                    self.logger.info(
                        "Locked on to a different target than destination??")
                    self.bot.hunter_locked_target = None

        if self.destination is not None:
            if self.destination_caught():
                self.logger.info(
                    "We found a %(name)s while hunting.",
                    self.destination)
        ...
        if self.destination is not None and self.config_hunt_for_trash:
            target_mons = filter(
                lambda x: x["name"] is self.destination["name"], pokemons)
                ...
                if len(pokemons) > 0:
                    if len(target_mons) < 1:
                        ...
                        distance = great_circle(
                            self.bot.position, (self.walker.dest_lat, self.walker.dest_lng)).meters
                        if (distance > 125 and self.lost_map_counter >
                                4) or self.lost_map_counter > 10:
                            # If > 120 meter => must be gone?
                            # Searching for 10 times, give up...
                            self.logger.info(
                                "It seems %(name)s is no longer there!", self.destination)
                            self.destination = None
                            self.hunting_trash = False
                            self.bot.hunter_locked_target = None
                            self.lost_map_counter = 0
        ...
        if self.config_hunt_for_trash and self.hunting_trash:
            if self.pokemon_slots_left() > self.config_trash_hunt_open_slots:
                self.logger.info(
                    "No longer trying to fill the bag. Electing new target....")
                self.hunting_trash = False
                self.destination = None
            # Closer target?
            if self.no_log_until < now:
                    ...
                    for pokemon in possible_targets:
                        if pokemon is not self.destination:
                            if round(
                                ...
                                break
                            self.logger.info(
                                "Found a closer target: %s < %s" %
                                (pokemon["distance"], self.destination["distance"]))
                            if self.destination is not None:
                                self.logger.info(
                                    "Closer trash hunt takes priority! Changing target...")
                            self.hunting_trash = True
                            self.destination = pokemon
                            self.lost_counter = 0
                            self.hunt_started_at = datetime.now()
                            self.logger.info(
                                "New target at %(distance).2f meters: %(name)s", self.destination)
                            self.set_target()
                            # We have a target
                            return WorkerResult.RUNNING

        if self.destination is None:
            worth_pokemons = self.get_worth_pokemons(
                pokemons, self.config_hunt_closest_first)
            ...
            if len(worth_pokemons) > 0:
                ...
                if self.previous_destination is not None:
                    ...
                    if self.previous_destination["pokemon_id"] == self.destination["pokemon_id"]:
                        ...
                        if "fort_id" in self.previous_destination and "fort_id" in self.destination and \
                            ...
                            self.previous_destination["distance"] = self.get_distance(
                                self.bot.start_position, self.previous_destination)
                            self.destination["distance"] = self.get_distance(
                                self.bot.start_position, self.destination)
                            if round(
                                    self.previous_destination["distance"], 2) == round(
                                    self.destination["distance"], 2):
                                self.logger.info(
                                    "Likely we are trying the same Pokemon again")
                                if len(worth_pokemons) > 1:
                                    self.destination = worth_pokemons[1]
                ...
                self.logger.info(
                    ...
                    self.destination)
                if self._is_vip_pokemon(
                        self.destination) and self.config_lock_on_target:
                    self.logger.info(
                        "This is a VIP Pokemon! Locking on to target!")
                ...
                if self.no_log_until < now:
                    ...
                    if len(names) > 0:
                        ...
                        self.logger.info(
                            "No sightings available at the moment...")
        ...
        if self.config_treat_family_of_vip_as_vip and self.destination is not None:
            if self._is_family_of_vip(self.destination):
                # We're hunting for family, so we need to check if we find a
                # VIP
                if self.no_log_until < now:
                    ...
                    if len(possible_targets) > 0:
                        ...
                        if possible_targets[0]["name"] != self.destination["name"]:
                            self.logger.info(
                                "We found a real VIP while hunting for %(name)s", self.destination)
                ...
                for pokemon in worth_pokemons:
                    if self._is_vip_pokemon(pokemon):
                        ...
                        if self.config_lock_on_target:
                            self.bot.hunter_locked_target = self.destination
                        self.logger.info(
                            "Spotted a VIP Pokemon! Looking for a %(name)s at %(distance).2f.",
                            self.destination)
                        return WorkerResult.SUCCESS
        ...
        if self.config_lock_on_target and not self.config_lock_vip_only:
            if self.bot.hunter_locked_target is None:
                self.logger.info(
        ...
        if any(self.destination["encounter_id"] == p["encounter_id"]
               for p in self.bot.cell["catchable_pokemons"] + self.bot.cell["wild_pokemons"]):
            self.destination = None
            self.hunting_trash = False
        elif self.walker.step(speed):
            if not any(self.destination["encounter_id"] ==
                       p["encounter_id"] for p in pokemons):
                self.lost_counter += 1
            else:
                self.lost_counter = 0
            ...
            if self.lost_counter >= 3:
                ...
                if self.search_points == []:
                    ...
                    self.walker = StepWalker(
                        self.bot, self.search_points[0][0], self.search_points[0][1])
                    self.search_points = self.search_points[1:] + \
                        self.search_points[:1]
        elif self.no_log_until < now:
            distance = great_circle(
                self.bot.position,
                (self.walker.dest_lat,
                 self.walker.dest_lng)).meters
            if distance > self.config_max_distance:
                # More than max_distance?
                self.logger.info(
                    "More than %sm away? Must have sniped,"
                    " resetting destination!" %
                    self.config_max_distance)
                self.destination = None
                self.bot.hunter_locked_target = None
                return WorkerResult.SUCCESS
            ...
            if self.distance_counter is 3:
                # Try another walker
                self.logger.info(
                    "Having difficulty walking to target, changing walker!")
                if self.search_points == []:
                    self.logger.info(
            ...
            if self.distance_counter >= 6:
                ...
                if speed is not None:
                    self.emit_event(
                        ...
                        data={
                            'target_name': u"{}".format(
                                self.destination["name"]),
                            'distance': format_dist(
                                distance,
                                unit),
                        })
                else:
    ...
    def set_target(self):
        if 's2_cell_id' not in self.destination:
            # This Pokemon has coords
            self.search_points = []
            self.walker = PolylineWalker(
                self.bot,
                self.destination["latitude"],
                self.destination["longitude"])
    ...
    def get_nearby_pokemons(self):
        ...
        pokemons = [p for p in self.bot.cell["nearby_pokemons"]
        ...
        for pokemon in pokemons:
            if "pokemon_data" in pokemon:
                ...
                pokemon["name"] = inventory.pokemons(
                ).name_for(pokemon["pokemon_id"])
    ...
    def _is_vip_pokemon(self, pokemon):
        ...
        if self.bot.config.vips.get(
            pokemon["name"]) == {} or (
            self.config_treat_unseen_as_vip and not inventory.pokedex().seen(
                pokemon["pokemon_id"])):
            return True
        # If we must treat the family of the Pokemon as a VIP, also return
        # true!
    ...
    def get_worth_pokemons(self, pokemons, closest_first=False):
        if self.config_hunt_all:
            ...
            worth_pokemons += [
                p for p in pokemons if not inventory.pokedex().seen(
                    p["pokemon_id"])]

            if self.config_hunt_vip:
                worth_pokemons += [p for p in pokemons if p["name"]
                                   in self.bot.config.vips]

            if self.config_target_family_of_vip or self.config_treat_family_of_vip_as_vip:
                worth_pokemons += [
                    p for p in pokemons if (
                        p not in worth_pokemons) and self._is_family_of_vip(p)]

            if self.config_hunt_pokedex:
                worth_pokemons += [
                    p for p in pokemons if (
                        p not in worth_pokemons) and self._is_needed_pokedex(p)]

        if closest_first:
            worth_pokemons.sort(key=lambda p: p["distance"])
        else:
            worth_pokemons.sort(
                key=lambda p: inventory.candies().get(
    ...
    def get_distance(self, location, pokemon):
        return great_circle(
            location, (pokemon["latitude"], pokemon["longitude"])).meters
    ...
    def get_search_points(self, cell_id):
        ...
        closest = min(
            points, key=lambda p: great_circle(
                self.bot.position, p).meters)
    ...
    def destination_vanished(self):
        ...
        with self.bot.database as conn:
            c = conn.cursor()
            c.execute(
                "SELECT COUNT(pokemon) FROM vanish_log where pokemon = '{}' and  datetime(dated, 'localtime') > Datetime('{}')".format(
                    self.destination["name"],
                    self.hunt_started_at.strftime("%Y-%m-%d %H:%M:%S")))
        # Now check if there is 1 or more caught
        amount = c.fetchone()[0]
    ...
    def get_nearest_fort_on_the_way(self):
        ...
        while i < len(forts):
            ...
            dist_self_to_fort = distance(
                self.bot.position[0],
                self.bot.position[1],
                forts[i]['latitude'],
                forts[i]['longitude'])
            dist_fort_to_pokemon = distance(
                self.search_points[0][0],
                self.search_points[0][1],
                forts[i]['latitude'],
                forts[i]['longitude'])
            total_dist = dist_self_to_fort + dist_fort_to_pokemon
            dist_self_to_pokemon = distance(
                self.bot.position[0],
                self.bot.position[1],
                self.search_points[0][0],
                self.search_points[0][1])
