...
class Sniper(BaseTask):
    ...
    def is_snipeable(self, pokemon):
        ...
        if pokemon.get('pokemon_name', '') in self.catch_list or self.mode == SniperMode.TELEGRAM:
            ...
            if self.mode != SniperMode.SOCIAL and pokemon.get('iv', 0) and pokemon.get('iv', 0) <= self.special_iv:
                self._log('{} is not catchable, but has a decent IV!'.format(pokemon.get('pokemon_name')))
            else:
