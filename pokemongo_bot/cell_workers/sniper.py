...
class SniperSource(object):
    ...
    def fetch_raw(self):
        ...
        # If the results is a dict, retrieve the list from it by the given key.
        # This will return a list afterall.
        if isinstance(results, dict):
            results = results.get(self.key, [])

        # If results is STILL a dict (eg. each pokemon is its own dict), need
        # to build data from nested json (example whereispokemon.net)
    ...
    def fetch(self):
        ...
        try:
            ...
            for result in results:
                ...
                name = self._get_closest_name(self._fixname(
                    result.get(self.mappings.name.param)))
                ...
                # If this is a composite param, split it ("coords":
                # "-31.415553, -64.190480")
                if self.mappings.latitude.param == self.mappings.longitude.param:
                    position = result.get(
                        self.mappings.latitude.param).replace(
                        " ", "").split(",")
                    latitude = position[0]
                ...
                # Format the time accordingly. Pokemon times are in
                # milliseconds!
                if self.mappings.expiration.exists and expiration:
                    if self.mappings.expiration.format == SniperSourceMappingTimeFormat.SECONDS:
                        ...
                        utc_date = datetime.strptime(
                            expiration.replace("T", " ")[:19], self.time_mask)
                        ...
                        local_date = local_date.replace(
                            microsecond=utc_date.microsecond)
                        expiration = time.mktime(local_date.timetuple()) * 1000
                else:
                    minutes_to_expire = 3
                    seconds_per_minute = 60
                    expiration = (
                        time.time() + minutes_to_expire * seconds_per_minute) * 1000
    ...
    def validate(self):
        try:
            if self.enabled:
                ...
                # Check whether the params really exist if they have been
                # specified like so
                if data:
                    ...
                    if self.mappings.latitude.exists and self.mappings.latitude.param not in data[
                            0]:
                        errors.append(self.mappings.latitude.param)
                    if self.mappings.longitude.exists and self.mappings.longitude.param not in data[
                            0]:
                        errors.append(self.mappings.longitude.param)
                    if self.mappings.expiration.exists and self.mappings.expiration.param not in data[
                            0]:
                        errors.append(self.mappings.expiration.param)
                    if self.mappings.encounter.exists and self.mappings.encounter.param not in data[
                            0]:
                        errors.append(self.mappings.encounter.param)
                    if self.mappings.spawnpoint.exists and self.mappings.spawnpoint.param not in data[
                            0]:
                        errors.append(self.mappings.spawnpoint.param)

                    # All wrong mappings were gathered at once for a better
                    # usability (instead of raising multiple exceptions)
                    if errors:
                        raise LookupError(
                            "The following params dont exist: {}".format(
                                ", ".join(errors)))
            else:
...
class SniperSourceMapping(object):
    def __init__(self, mapping):
        ...
        self.latitude = SniperSourceMappingValues(
            True, mapping.get('latitude', {}))
        self.longitude = SniperSourceMappingValues(
            True, mapping.get('longitude', {}))
        self.expiration = SniperSourceMappingValues(
            False, mapping.get('expiration', {}))
        self.encounter = SniperSourceMappingValues(
            False, mapping.get('encounter', {}))
        self.spawnpoint = SniperSourceMappingValues(
            False, mapping.get('spawnpoint', {}))
...
class SniperSourceMappingValues(object):
    def __init__(self, required, values):
        ...
        self.format = values.get(
            'format', SniperSourceMappingTimeFormat.DEFAULT)
        self.exists = values != {} and values.get('param') is not None
...
# Teleports the player to a target gotten from either social or a
# single/multiple URL sources
...
class Sniper(BaseTask):
    ...
    def initialize(self):
        ...
        self.altitude = uniform(
            self.bot.config.alt_min,
            self.bot.config.alt_max)
        self.sources = [SniperSource(data)
                        for data in self.config.get('sources', [])]
        ...
        if self.enabled:
            ...
            if self.mode not in vars(SniperMode).values():
                ...
                if self.mode == SniperMode.URL:
                    self._log("Validating sources: {}...".format(
                        ", ".join([source.url for source in self.sources])))

                    # Create a copy of the list so we can iterate and remove
                    # elements at the same time
                    for source in list(self.sources):
                        try:
                            source.validate()
                            self._log(
                                "Source '{}' is good!".format(
                                    source.url))
                        # TODO: On ValueError, remember source and validate
                        # later (pending validation)
                        except (LookupError, ValueError) as exception:
                            self._error(
                                "Source '{}' contains errors. Details: {}. Removing from sources list...".format(
    ...
    def is_snipeable(self, pokemon):
        ...
        # Skip if expired (cast milliseconds to seconds for comparision), snipe
        # check if source is from telegram
        if self.mode != SniperMode.TELEGRAM:
            if (pokemon.get('expiration_timestamp_ms', 0) or pokemon.get(
                    'last_modified_timestamp_ms', 0)) / 1000 < time.time():
                self._trace(
                    '{} is expired! Skipping...'.format(
                        pokemon.get('pokemon_name')))
                return False

        # Skip if not enought balls. Sniping wastes a lot of balls. Theres no
        # point to let user decide this amount
        if all_balls_count < self.MIN_BALLS_FOR_CATCHING:
            self._trace('Not enought balls left! Skipping...')
            return False

        # Skip if not in catch list, not a VIP and/or IV sucks (if any), ignore
        # telegram mode
        if pokemon.get(
            'pokemon_name',
                '') in self.catch_list or self.mode == SniperMode.TELEGRAM:
            self._trace('{} is catchable!'.format(pokemon.get('pokemon_name')))
        else:
            # Not catchable. Having a good IV should suppress the not in
            # catch/vip list (most important)
            if pokemon.get(
                    'iv', 0) and pokemon.get(
                    'iv', 0) >= self.special_iv:
                self._trace(
                    '{} is not catchable, but has a decent IV ({})!'.format(
                        pokemon.get('pokemon_name'), pokemon.get(
                            'iv', 0)))
            else:
                # Not catchable and IV is not good enough (if any). Check VIP
                # list
                if pokemon.get('vip', False):
                    self._trace(
                        '{} is not catchable and bad IV (if any), however its a VIP!'.format(
                            pokemon.get('pokemon_name')))
                else:
                    if pokemon.get('missing', False):
                        self._trace(
                            '{} is not catchable, not VIP and bad IV (if any), however its a missing one.'.format(
                                pokemon.get('pokemon_name')))
                    else:
                        self._trace(
                            '{} is not catchable, nor a VIP or a missing one and bad IV (if any). Skipping...'.format(
                                pokemon.get('pokemon_name')))
                        return False

        return True

    # Snipe a target. This function admits that if a target really exists, it
    # will be 'caught'.
    def snipe(self, pokemon):
        success = False

        # Apply snipping business rules and snipe if its good
        if not self.is_snipeable(
                pokemon) and not self.mode == SniperMode.TELEGRAM:
            self._trace(
                '{} is not snipeable! Skipping...'.format(
            ...
            if self._is_cached(uniqueid):
                # Do nothing. Either we already got this, or it doesn't really
                # exist
                self._trace(
                    '{} was already handled! Skipping...'.format(
                        pokemon['pokemon_name']))
            else:
                ...
                # If social is enabled and if no verification is needed, trust
                # it. Otherwise, update IDs!
                verify = not pokemon.get(
                    'encounter_id') or not pokemon.get('spawn_point_id')
                exists = not verify or self.mode == SniperMode.SOCIAL
                success = exists

                # Always verify if it's from telegram
                if TelegramSnipe.ENABLED:
                    verify = True
                ...
                if verify:
                    ...
                    # Wait a maximum of MIN_SECONDS_ALLOWED_FOR_CELL_CHECK
                    # seconds before requesting nearby cells
                    self._trace(
                        'Pausing for {} secs before checking for Pokemons'.format(
                            self.MIN_SECONDS_ALLOWED_FOR_CELL_CHECK))

                    # recode it to check every 5 secs, first check for wild
                    # then catchable
                    nearby_pokemons = []
                    nearby_stuff = []
                    num = 0
                    for num in range(
                            0, self.MIN_SECONDS_ALLOWED_FOR_CELL_CHECK):
                        if num % 5 == 0:
                            nearby_stuff = self.bot.get_meta_cell()
                            self.last_cell_check_time = time.time()

                            # Retrieve nearby pokemons for validation
                            nearby_pokemons.extend(
                                nearby_stuff.get('wild_pokemons', []))
                            if nearby_pokemons:
                                break
                    ...
                    for num in range(
                        ...
                        if num % 5 == 0:
                            ...
                            # Retrieve nearby pokemons for validation
                            nearby_pokemons.extend(
                                nearby_stuff.get('catchable_pokemons', []))
                            if nearby_pokemons:
                                break
                    ...
                    # Make sure the target really/still exists (nearby_pokemon
                    # key names are game-bound!)
                    for nearby_pokemon in nearby_pokemons:
                        nearby_pokemon_id = nearby_pokemon.get('pokemon_data', {}).get(
                            'pokemon_id') or nearby_pokemon.get('pokemon_id')

                        # If we found the target, it exists and will very
                        # likely be encountered/caught (success)
                        if nearby_pokemon_id == pokemon.get('pokemon_id', 0):
                            exists = True
                            success = True

                            # Also, if the IDs arent valid, override them
                            # (nearby_pokemon key names are game-bound!) with
                            # game values
                            if not pokemon.get('encounter_id') or not pokemon.get(
                                    'spawn_point_id'):
                ...
                if exists:
                    self._log(
                        'Yay! There really is a wild {} nearby!'.format(
                            pokemon.get('pokemon_name')))
                    self._teleport_back_and_catch(last_position, pokemon)

                else:
                    self._error(
                        'Damn! Its not here. Reasons: too far, caught, expired or fake data. Skipping...')
                    self._teleport_back(last_position)

                # Set always to false to re-enable sniper to check for telegram
                # data
    ...
    def work(self):
        ...
        if self.no_snipe_until is not None and self.no_snipe_until > time.time():
        ...
        if self.bot.softban:
            if not hasattr(
                self.bot,
                "sniper_softban_global_warning") or (
                hasattr(
                    self.bot,
                    "sniper_softban_global_warning") and not self.bot.sniper_softban_global_warning):
        ...
        if self.disabled:
            ...
            if not hasattr(
                self.bot,
                "sniper_disabled_global_warning") or (
                hasattr(
                    self.bot,
                    "sniper_disabled_global_warning") and not self.bot.sniper_disabled_global_warning):
                self._log(
                    "All catching tasks are currently disabled until {}. Sniper will resume when catching tasks are re-enabled".format(
            ...
            if targets:
                ...
                targets = sorted(
                    targets, key=itemgetter(
                        *self.order), reverse=True)
                # List Pokemons found
                self._trace(
                    'Sniping the {} best Pokemons found, ordered by {}'.format(
                        self.bullets, self.order))
                self._trace(
                    '+----+------+----------------+-------+----------+---------+---------+----------+')
                self._trace(
                    '|  # |   Id | Name           |    IV | Verified | VIP     | Missing | Priority |')
                self._trace(
                    '+----+------+----------------+-------+----------+---------+---------+----------+')
                row_format = "|{:>3} |{:>5} | {:<15}|{:>6} | {:<9}| {:<8}| {:<8}|{:>9} |"
                for index, target in enumerate(targets):
                    self._trace(row_format.format(*[index + 1,
                                                    target.get('pokemon_id'),
                                                    target.get('pokemon_name'),
                                                    target.get('iv'),
                                                    str(target.get('verified')),
                                                    str(target.get('vip')),
                                                    str(target.get('missing')),
                                                    target.get('priority')]))
                ...
                # For as long as there are targets available, try to snipe
                # untill we run out of bullets
                for index, target in enumerate(targets):
                    sniped = True
                    if shots < self.bullets:
                        ...
                        # Homing shots are supposed to hit the target
                        # (capture). Rollback
                        if self.homing_shots and not success:
                            shots -= 1

                        # Wait a bit if were going to snipe again (bullets and
                        # targets left)
                        if shots < self.bullets and index < len(targets):
                            self._trace(
                                'Waiting a few seconds to teleport again to another target...')
                            time.sleep(3)
        ...
        if sniped:
            if self.loiter_after_snipe:
                ...
                self._log(
                    "Loitering for {} seconds after sniping to allow Niantic flags to drop off...".format(loiter))
                time.sleep(loiter)
            if self.cooldown_enabled:
                wait = uniform(60, 360)
                self.no_snipe_until = time.time() + wait
                self._log("Snipe on cooldown until {}.".format(
                    (datetime.now() + timedelta(seconds=wait)).strftime("%H:%M:%S")))
    ...
    def _parse_pokemons(self, pokemon_dictionary_list):
        ...
        # Build up the pokemon. Pops are used to destroy random attribute names
        # and keep the known ones!
        for pokemon in pokemon_dictionary_list:
            pokemon['iv'] = pokemon.get('iv', 100)
            pokemon['pokemon_name'] = pokemon.get(
                'pokemon_name', Pokemons.name_for(
                    pokemon.get('pokemon_id')))
            pokemon['vip'] = pokemon.get(
                'pokemon_name') in self.bot.config.vips
            pokemon['missing'] = not self.pokedex.captured(
                pokemon.get('pokemon_id'))
            pokemon['priority'] = self.catch_list.get(
                pokemon.get('pokemon_name'), 0)
    ...
    def _get_pokemons_from_telegram(self):
        ...
        pokemons = []
        pokemon = {
            'iv': int(0), 'pokemon_id': int(
                TelegramSnipe.ID), 'pokemon_name': str(
                TelegramSnipe.POKEMON_NAME), 'latitude': float(
                TelegramSnipe.LATITUDE), 'longitude': float(
                    TelegramSnipe.LONGITUDE)}
        self._log(
            'Telegram snipe request: {}'.format(
                pokemon.get('pokemon_name')))
    ...
    def _get_pokemons_from_social(self):
        if not hasattr(
                self.bot,
                'mqtt_pokemon_list') or not self.bot.mqtt_pokemon_list:
            return []

        self._trace('Social has returned {} pokemon(s)'.format(
            len(self.bot.mqtt_pokemon_list)))
    ...
    def _get_pokemons_from_url(self):
        ...
        # If something is requesting this info too fast, skip it (we might
        # crash their servers)
        if (seconds_since_last_valid_request >
                self.MIN_SECONDS_ALLOWED_FOR_REQUESTING_DATA):
            self.last_data_request_time = time.time()
            ...
            for source in self.sources:
                try:
                    if source.enabled:
                        ...
                        self._trace(
                            "Source '{}' returned {} results".format(
                                source.url, len(source_pokemons)))

                        # Merge lists, making sure to exclude repeated data.
                        # Use location as the hash key
                        for source_pokemon in source_pokemons:
                            hash_key = self._hash(source_pokemon)

                            # Add if new
                            if hash_key not in results_hash_map:
                                results_hash_map[hash_key] = source_pokemon
                    else:
                        self._trace(
                            "Source '{}' is disabled".format(
                                source.url))
                except Exception as exception:
                    self._error(
                        "Could not fetch data from '{}'. Details: {}. Skipping...".format(
                            source.url, exception))
            self._trace("After merging, we've got {} results".format(
                len(results_hash_map.values())))
        else:
    ...
    def _hash(self, pokemon):
        # Use approximate location instead, because some IDs might be wrong.
        # The first 4 decimal places is enough for this
        return "{0:.4f};{1:.4f}".format(
            pokemon.get('latitude'),
            pokemon.get('longitude'))
    ...
    def _build_unique_id(self, pokemon):
        # Build unique id for this pokemon from id, latitude, longitude and
        # expiration
        uniqueid = str(pokemon.get('pokemon_id',
                                   '')) + str(pokemon.get('latitude',
                                                          '')) + str(pokemon.get('longitude',
                                                                                 '')) + str(pokemon.get('expiration',
                                                                                                        ''))
        md5str = hashlib.md5()
