...
class SpinFort(BaseTask):
    ...
    def work(self):
        ...
        if not is_gym:  # can't lure a gym!!
                ...
                if lure_count > 1:  # Only use lures when there's more than one
                    ...
                    if ('responses' in response_dict) and (
                        ...
                        if (add_modifier_result == LURE_REQUEST_RESULT_SUCCESS):
                            self.emit_event(
                                'lure_success', formatted='You have successfully placed a lure')
                        if (add_modifier_result ==
                                LURE_REQUEST_FORT_ALREADY_HAS_MODIFIER):
                            self.emit_event(
                                'lure_failed', formatted='A lure has being placed before you try to do so')
                        ...
                        if (add_modifier_result ==
                            ...
                            self.emit_event(
                                'lure_not_enough', formatted='Not enough lure in inventory')
            ...
            # When it's a gym, we need to fake tapping on the gym before a spin
            self.get_gym_details(fort)
    ...
    def get_forts_in_range(self):
        ...
        forts = filter(lambda fort: fort["id"]
                       not in self.bot.fort_timeouts, forts)
        if hasattr(
                self.bot,
                "camping_forts") and self.bot.camping_forts and self.try_to_keep_streak:
            if datetime.now() >= self.next_update:
            ...
            # Remove all forts which were spun in the last ticks to keep 10
            # stops streak
    ...
    def get_items_awarded_from_fort_spinned(self, response_dict):
        experience_awarded = response_dict['responses'][
            'FORT_SEARCH'].get('experience_awarded', 0)
        inventory.player().exp += experience_awarded

        items_awarded = response_dict['responses'][
            'FORT_SEARCH'].get('items_awarded', {})
        loot = response_dict['responses']['FORT_SEARCH'].get('loot', {})
        bonus_loot = response_dict['responses'][
            'FORT_SEARCH'].get('bonus_loot', {})
        team_bonus_loot = response_dict['responses'][
            'FORT_SEARCH'].get('team_bonus_loot', {})
        tmp_count_items = {}
    ...
    # TODO : Refactor this class, hide the inventory update right after the
    # api call
