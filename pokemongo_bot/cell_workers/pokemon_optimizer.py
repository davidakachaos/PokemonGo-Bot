...
class PokemonOptimizer(BaseTask):
    ...
    def initialize(self):
        ...
        pokemon_upgrade_cost_file = os.path.join(
            _base_dir, "data", "pokemon_upgrade_cost.json")
        ...
        if self.debug:
            log_file_path = os.path.join(
                _base_dir, "data", "pokemon-optimizer-%s.log" % self.bot.config.username)
        ...
        self.config_bulktransfer_enabled = self.config.get(
            "bulktransfer_enabled", False)
        self.config_use_evolution_items = self.config.get(
            "use_evolution_items", False)
        ...
        self.config_transfer_after_xp_evolve = self.config.get(
            "transfer_after_xp_evolve", True)
        ...
        self.config_may_evolve_favorites = self.config.get(
            "may_evolve_favorites", True)
        self.config_may_upgrade_favorites = self.config.get(
            "may_upgrade_favorites", True)
        self.config_may_unfavor_pokemon = self.config.get(
        ...
        self.config_rules = self.config.get("rules", [{"mode": "overall", "top": 1, "sort": ["max_cp", "cp"], "keep": {"candy": -124}, "evolve": False, "buddy": True},
                                                      {"mode": "overall", "top": 1, "sort": [
                                                          "-candy", "max_cp", "cp"], "evolve": False, "buddy": True},
                                                      {"mode": "by_family", "top": 3, "names": ["gym"], "sort": ["iv", "ncp"], "evolve": {
                                                          "iv": 0.9, "ncp": 0.9}, "upgrade": {"iv": 0.9, "ncp": 0.9}},
                                                      {"mode": "by_family", "top": 1, "sort": [
                                                          "iv"], "evolve": {"iv": 0.9}},
                                                      {"mode": "by_family", "top": 1, "sort": [
                                                          "ncp"], "evolve": {"ncp": 0.9}},
                                                      {"mode": "by_family", "top": 1, "sort": [
        ...
        self.config_evolve_for_xp_whitelist, self.config_evolve_for_xp_blacklist = self.get_colorlist(
            self.config_evolve_for_xp)
        ...
        for pokemon in inventory.Pokemons.STATIC_DATA:
            ...
            if pokemon.prev_evolutions_all:
                self.config_groups[
                    "with_previous_evolution"].append(pokemon.name)
    ...
    def get_pokemon_slot_left(self):
        ...
        if pokemon_count != self.last_pokemon_count:
            ...
            self.logger.info("Pokemon Bag: %s / %s",
                             pokemon_count, self.max_pokemon_storage)
        ...
        for _ in itertools.repeat(None, 2):
            ...
            for rule in self.config_rules:
                ...
                if check_top == "all" and names == [] and check_keep:
                    self.logger.info(
                        "WARNING!! Will not transfer any Pokemon!!")
                    self.logger.info(rule)
                    self.logger.info(
                        "This rule is set to keep (`keep` is true) all Pokemon (no `top` and no `names` set!!)")
                ...
                if mode == "by_pokemon":
                    for pokemon_id, pokemon_list in self.group_by_pokemon_id(inventory.pokemons().all()):
                        ...
                        keep, try_evolve, try_upgrade, buddy, favor = self.get_best_pokemon_for_rule(
                            sorted_list, rule)
                        keep_all += keep
                    ...
                    for family_id, pokemon_list in self.group_by_family_id(inventory.pokemons().all()):
                        ...
                        if family_id == 133:  # "Eevee"
                            keep, try_evolve, try_upgrade, buddy, favor = self.get_multi_best_pokemon_for_rule(
                                sorted_list, rule, 3)
                        else:
                            keep, try_evolve, try_upgrade, buddy, favor = self.get_best_pokemon_for_rule(
                                sorted_list, rule)
            ...
            if self.config_may_unfavor_pokemon:
                ...
                if len(unfavor) > 0:
                    self.logger.info(
                        "Marking %s Pokemon as no longer favorite", len(unfavor))
            ...
            if len(try_favor_all) > 0:
                self.logger.info(
                    "Marking %s Pokemon as favorite", len(try_favor_all))
            ...
            # Only check bag on the first run, second run ignores if the bag is
            # empty enough
            ...
            upgrade_limited_all = []
            for family_id, pokemon_list in self.group_by_family_id(inventory.pokemons().all()):
                keep = [p for p in keep_all if self.get_family_id(
                    p) == family_id]
                try_evolve = [
                    p for p in try_evolve_all if self.get_family_id(p) == family_id]
                try_upgrade = [
                    p for p in try_upgrade_all if self.get_family_id(p) == family_id]

                transfer, evolve, upgrade, xp, limited_upgrade = self.get_evolution_plan(
                    family_id, pokemon_list, keep, try_evolve, try_upgrade)
            ...
            self.apply_optimization(
                transfer_all, evolve_all, upgrade_all, xp_all, upgrade_limited_all)
    ...
    def check_buddy(self):
        ...
        pokemon = next((p for p in inventory.pokemons().all()
                        if p.unique_id == self.buddy["id"]), None)
        ...
        if distance_walked >= distance_needed:
            ...
            self.buddy["last_km_awarded"] = self.buddy[
                "start_km_walked"] + distance_needed * int(distance_walked / distance_needed)
            self.lock_buddy = False
        else:
            now = time.time()
    ...
    def open_inventory(self):
        for pokemon in inventory.pokemons().all():
            ...
            candy_to_evolution = max(
                pokemon.evolution_cost - pokemon.candy_quantity, 0)
    ...
    def get_closest_name(self, name):
        ...
        if closest_names:
            ...
            if name != closest_name:
                self.logger.warning(
                    "Unknown Pokemon name [%s]. Assuming it is [%s]", name, closest_name)
    ...
    def get_score(self, pokemon, rule):
        ...
        may_try_evolve = (hasattr(pokemon, "has_next_evolution")
                          and pokemon.has_next_evolution())
        ...
        if self.debug:
            self.log("P:%s S:%s K:%s E:%s U:%s B:%s F:%s" % (pokemon, tuple(
                score), keep, may_try_evolve, may_try_upgrade, may_buddy, may_favor))
    ...
    def get_multi_best_pokemon_for_rule(self, family_list, rule, nb_branch):
        ...
        senior_pokemon_list = [
            p for p in family_list if not p.has_next_evolution()]
        ...
        if not self.config_evolve:
            # Player handle evolution manually = Fall-back to per Pokemon
            # behavior
            ...
            if len(other_family_list) > 0:
                if len(senior_pids) < nb_branch:
                    # We did not get every combination yet = All other Pokemon
                    # are potentially good to keep
                ...
                keep, try_evolve, try_upgrade, buddy, favor = self.get_better_pokemon(
                    other_family_list, worst, 12)
                keep_all += keep
                try_evolve_all += try_evolve
                try_upgrade_all += try_upgrade
                buddy_all += buddy
    ...
    def get_better_pokemon(self, pokemon_list, worst, limit=1000):
        keep = [p for p in pokemon_list if p.__score__[
            0] >= worst.__score__[0]][:limit]
        try_evolve = [p for p in keep if p.__score__[2] is True]
        try_upgrade = [p for p in keep if (
            p.__score__[2] is False) and (p.__score__[3] is True)]
    ...
    def get_evolution_plan(self, family_id, family_list, keep, try_evolve, try_upgrade):
        ...
        crap = [p for p in crap if not p.in_fort and not p.is_favorite and not (
            p.unique_id == self.buddyid)]
        crap.sort(key=lambda p: (p.iv, p.cp), reverse=True)

        # We will gain a candy whether we choose to transfer or evolve these
        # Pokemon
        candies += len(crap)
        ...
        for pokemon in try_evolve:
            ...
            if needed_evolution_item is not None:
                if self.config_use_evolution_items:
                    ...
                    if item.count < needed:
                        self.logger.info("To evolve a {} we need {} of {}. We have {}".format(
                            pokemon.name, needed, item.name, item.count))
                        continue
                else:
            ...
            if self.config_evolve_to_final:
                ...
                while inventory.pokemons().has_next_evolution(pokemon_id):
                    ...
                    pokemon_id = inventory.pokemons(
                    ).next_evolution_ids_for(pokemon_id)[0]
            else:
        ...
        if max_cp_upgrade and int(max_cp_upgrade) > 0:
            if not self.config_may_upgrade_favorites:
                self.logger.info(
                    "Removing favorites from limited upgrade list.")
            ...
            for pokemon in try_upgrade:
                ...
                if number_of_powerups == 0:
                    ...
                    self.logger.info("Up a %s to level %s to get a CP of %s" % (
                        pokemon.name, pokemon_level, int(previous_cp)))
        ...
        if (not self.config_evolve_for_xp) or (family_name in self.config_evolve_for_xp_blacklist):
            ...
            # Compute how many crap we should keep if we want to batch evolve
            # them for xp
            ...
            xp = [p for p in crap if p.has_next_evolution(
            ) and p.evolution_cost == lowest_evolution_cost][:keep_for_xp]
    ...
    def apply_optimization(self, transfer, evolve, upgrade, xp, upgrade_limited_all):
        ...
        if self.config_upgrade or self.bot.config.test:
            if upgrade_count > 0:
                self.logger.info(
                    "Upgrading %s Pokemon [%s stardust]", upgrade_count, self.bot.stardust)
            ...
            if upgrade_limited_count > 0:
                self.logger.info(
                    "Upgrading %s Pokemon to a max of a certain CP [%s stardust]", upgrade_limited_count, self.bot.stardust)
        ...
        if self.config_evolve or self.bot.config.test:
            ...
            if evolve_xp_count > 0:
                ...
                if not skip_evolve:
                    ...
                    if evolve_count > 0:
                        self.logger.info(
                            "Evolving %s Pokemon (the best)", evolve_count)
                    ...
                    if xp_count > 0:
                        self.logger.info(
                            "Evolving %s Pokemon (for xp)", xp_count)

                        for pokemon in xp:
                            self.evolve_pokemon(
                                pokemon, self.config_transfer_after_xp_evolve)
            ...
            while len(pokemons) > 0:
                ...
                try:
                    if self.config_transfer:
                        ...
                        result = response_dict['responses'][
                            'RELEASE_POKEMON']['result']
                        if result != 1:
                            self.logger.error(
                                u'Error while transfer pokemon: {}'.format(error_codes[result]))
                            return False
                except Exception:
                    return False
                ...
                for pokemon in transfered:
                    ...
                    if self.config_transfer:
                        ...
                        with self.bot.database as db:
                            ...
                            if db_result[0] == 1:
                                db.execute("INSERT INTO transfer_log (pokemon, iv, cp) VALUES (?, ?, ?)", (
            ...
            for pokemon in pokemons:
                if self.config_transfer and (not self.bot.config.test):
                    ...
                    response_dict = {"responses": {
                        "RELEASE_POKEMON": {"candy_awarded": 0}}}

                if not response_dict:
                    return False

                candy_awarded = response_dict.get("responses", {}).get(
                    "RELEASE_POKEMON", {}).get("candy_awarded", 0)
                candy = inventory.candies().get(pokemon.pokemon_id)
    ...
    def use_lucky_egg(self):
        ...
        result = response_dict.get("responses", {}).get(
    ...
    def evolve_pokemon(self, pokemon, transfer=False):
        ...
        if pokemon.is_bad:
            self.logger.info(
                "This %s is a bad/slashed Pokemon! Can't evolve!" % pokemon.name)

        if pokemon.in_fort:
            self.logger.info(
                "This %s is in a gym! Can't evolve!" % pokemon.name)
        ...
        if self.config_evolve and (not self.bot.config.test):
            ...
            if needed_evolution_item is not None:
                if self.config_use_evolution_items:
                    ...
                    request.evolve_pokemon(
                        pokemon_id=pokemon.unique_id, evolution_item_requirement=needed_evolution_item)
                    response_dict = request.call()
                else:
                    return False
            else:
            ...
            response_dict = {"responses": {
                "EVOLVE_POKEMON": {"result": SUCCESS}}}

        if not response_dict:
            return False

        result = response_dict.get("responses", {}).get(
            "EVOLVE_POKEMON", {}).get("result", 0)
        ...
        xp = response_dict.get("responses", {}).get(
            "EVOLVE_POKEMON", {}).get("experience_awarded", 0)
        candy_awarded = response_dict.get("responses", {}).get(
            "EVOLVE_POKEMON", {}).get("candy_awarded", 0)
        candy = inventory.candies().get(pokemon.pokemon_id)
        evolution = response_dict.get("responses", {}).get(
            "EVOLVE_POKEMON", {}).get("evolved_pokemon_data", {})
        ...
        if self.config_evolve and (not self.bot.config.test):
            ...
            with self.bot.database as db:
                cursor = db.cursor()
                cursor.execute(
                    "SELECT COUNT(name) FROM sqlite_master WHERE type='table' AND name='evolve_log'")

                db_result = cursor.fetchone()

                if db_result[0] == 1:
                    db.execute("INSERT INTO evolve_log (pokemon, iv, cp) VALUES (?, ?, ?)",
                               (pokemon.name, pokemon.iv, pokemon.cp))
    ...
    def upgrade_pokemon(self, pokemon, target_level=None):
        ...
        for i in range(int(pokemon.level * 2), int(upgrade_level * 2)):
            ...
            if self.config_upgrade and (not self.bot.config.test):
                ...
                response_dict = {"responses": {
                    "UPGRADE_POKEMON": {"result": SUCCESS}}}

            if not response_dict:
                return False

            result = response_dict.get("responses", {}).get(
                "UPGRADE_POKEMON", {}).get("result", 0)
    ...
    def set_buddy_pokemon(self, pokemon):
        if not self.bot.config.test:
            ...
            response_dict = {"responses": {"SET_BUDDY_POKEMON": {
                "result": SUCCESS, "updated_buddy": {"start_km_walked": 0, "last_km_awarded": 0, "id": 0}}}}

        if not response_dict:
            return False

        result = response_dict.get("responses", {}).get(
            "SET_BUDDY_POKEMON", {}).get("result", 0)

        if result != SUCCESS:
            return False

        if not self.bot.config.test:
            self.buddy = response_dict.get("responses", {}).get(
                "SET_BUDDY_POKEMON", {}).get("updated_buddy", {})
            self.buddyid = self._get_buddyid()
    ...
    def get_buddy_walked(self, pokemon):
        if not self.bot.config.test:
            ...
            response_dict = {"responses": {"GET_BUDDY_WALKED": {
                "success": True, "family_candy_id": 0, "candy_earned_count": 0}}}

        if not response_dict:
            return False

        success = response_dict.get("responses", {}).get(
            "GET_BUDDY_WALKED", {}).get("success", False)
    ...
    def favor_pokemon(self, pokemon):
        ...
        request.set_favorite_pokemon(
            pokemon_id=pokemon.unique_id, is_favorite=True)
        response_dict = request.call()

        sleep(1.2)  # wait a bit after request
        if response_dict:
            result = response_dict.get('responses', {}).get(
                'SET_FAVORITE_POKEMON', {}).get('result', 0)
            if result is 1:  # Request success
                ...
                pokemon.is_favorite = True
                self.logger.info(
                    "Unable to set %s as favorite!" % pokemon.name)

    def unfavor_pokemon(self, pokemon):
        return True
        request = self.bot.api.create_request()
        request.set_favorite_pokemon(
            pokemon_id=pokemon.unique_id, is_favorite=False)
        response_dict = request.call()
